...
keys = [
    ...
    Key([mod, "shift"], "m", lazy.spawn("theme-switcher"), desc="Open theme switcher"),
...
Load the theme sets by the theme switcher

#+begin_src python
import importlib.util

def load_current_theme():
    """Load the current theme from the theme switcher system"""
    theme_file = os.path.expanduser("~/.config/qtile/current_theme.py")

    if os.path.exists(theme_file):
        try:
            spec = importlib.util.spec_from_file_location("current_theme", theme_file)
            theme_module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(theme_module)

            # Find the theme dictionary in the module
            for attr_name in dir(theme_module):
                attr = getattr(theme_module, attr_name)
                if (isinstance(attr, dict) and
                    'bg' in attr and 'fg' in attr and
                    not attr_name.startswith('__')):
                    return attr
        except Exception as e:
            print(f"Error loading theme: {e}")

    # Fallback to Gruvbox if theme loading fails
    return {
        "bg":      "#282c34",
        "fg":      "#bbc2cf",
        "black":   "#1c1f24",
...
#+begin_src python
colors = load_current_theme()
#+end_src

Hook to reload the config on theme change

#+begin_src python
@hook.subscribe.client_name_updated
def reload_config_on_theme_change(client):
    """Reload config when theme files change"""
    if client.name and "theme-switcher" in client.name:
        qtile.call_later(1, lambda: qtile.cmd_reload_config())
#+end_src
...
def create_menu():
    menu_items = [
        ("Shutdown now", lazy.spawn("shutdown now")),
